class StatesController < ApplicationController
  ...
  def states
    @states_array = State.where(:country => params[:country]).map{ |state| [state.state, state.id]}
    render :layout => false
  end
  ...
  private
    ...
    def check_authorization
      if !current_user.is_admin?
        redirect_to root_path, :notice => "You don't have access to add state"
      end
    end
    ...
    def state_params
      params.require(:state).permit(:country, :state)
    end
end
