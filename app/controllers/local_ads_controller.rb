class LocalAdsController < ApplicationController
  ...
  def index
    if params[:category].present? && params[:category] != "All"
      ...
      @local_ads = LocalAd.all.select{|local_ad| local_ad.user == current_user || local_ad.user.zip.downcase == current_user.zip.downcase || local_ad.user.sub_region.downcase == current_user.sub_region.downcase || local_ad.user.street.downcase == current_user.street.downcase}
    end
    @desc_length = 60
    @title_length = 40
